...
class ReviewArrivalNotificationErrorsP5ControllerSpec extends SpecBase with AppWithDefaultMockFixtures with ScalaCheckPropertyChecks with Generators {

  private val mockReviewArrivalNotificationErrorMessageP5ViewModelProvider = mock[ReviewArrivalNotificationErrorsP5ViewModelProvider]
  ...
  lazy val rejectionMessageController: String = controllers.testOnly.routes.ReviewArrivalNotificationErrorsP5Controller.onPageLoad(departureIdP5).url
  val sections: Seq[Section]                  = arbitrarySections.arbitrary.sample.value

  override def beforeEach(): Unit = {
    super.beforeEach()
    reset(mockArrivalP5MessageService)
    reset(mockReviewArrivalNotificationErrorMessageP5ViewModelProvider)
  ...
  override def guiceApplicationBuilder(): GuiceApplicationBuilder =
    super
      ...
      .overrides(bind[ReviewArrivalNotificationErrorsP5ViewModelProvider].toInstance(mockReviewArrivalNotificationErrorMessageP5ViewModelProvider))
      .overrides(bind[ArrivalP5MessageService].toInstance(mockArrivalP5MessageService))

  "ReviewArrivalNotificationErrorsP5Controller" - {
    ...
    "must return OK and the correct view for a GET" in {
      ...
      when(mockReviewArrivalNotificationErrorMessageP5ViewModelProvider.apply(any(), any())(any(), any(), any()))
        .thenReturn(Future.successful(ReviewArrivalNotificationErrorsP5ViewModel(sections, mrn, multipleErrors = true)))

      rejectionMessageAction(departureIdP5, mockArrivalP5MessageService)

      val rejectionMessageP5ViewModel = new ReviewArrivalNotificationErrorsP5ViewModel(sections, mrn, true)
