...
class IntentionToControlIndexControllerSpec extends SpecBase with ScalaCheckPropertyChecks with Generators {
  ...
  "IntentionToControlIndexController" - {
    ...
    s"when notification type 2 and no control information requested - must redirect to noInformationRequested controller" in {
      ...
      val request = FakeRequest(GET, controllers.departureP5.routes.IntentionToControlIndexController.onPageLoad(departureIdP5, messageId).url)
