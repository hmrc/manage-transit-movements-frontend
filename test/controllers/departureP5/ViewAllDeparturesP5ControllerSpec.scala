...
class ViewAllDeparturesP5ControllerSpec extends SpecBase with AppWithDefaultMockFixtures with ScalaCheckPropertyChecks with Generators {
  ...
  "ViewAllDeparturesP5Controller" - {
    ...
    "must return a Bad Request and errors when invalid data is submitted (POST)" in {
      ...
      val controllerUrl = routes.ViewAllDeparturesP5Controller.onSubmit(Some(page)).url
      ...
      val view      = injector.instanceOf[ViewAllDeparturesP5View]
      val viewModel = ViewAllDepartureMovementsP5ViewModel(departures, None, page, 20, movements.totalCount)
