...
class DeclarationAmendmentRejectionMessageControllerSpec extends SpecBase with AppWithDefaultMockFixtures with ScalaCheckPropertyChecks with Generators {
  ...
  lazy val declarationAmendmentRejectionMessageOnPageLoadRoute: String =
    routes.DeclarationAmendmentRejectionMessageController.onPageLoad(None, departureIdP5, messageId).url

  lazy val declarationAmendmentRejectionMessageOnSubmitRoute: String =
    routes.DeclarationAmendmentRejectionMessageController.onSubmit(departureIdP5, messageId).url
  ...
  "DeclarationAmendmentRejectionMessageController" - {
    ...
    "must return OK and the correct view for a GET" in {
      ...
      forAll(arbitrary[CC022CType], arbitrary[FunctionalErrorsWithSection]) {
        (message, functionalErrors) =>
          ...
          val request = FakeRequest(GET, declarationAmendmentRejectionMessageOnPageLoadRoute)

          val result = route(app, request).value

          status(result) mustEqual OK

          val view = injector.instanceOf[DeclarationAmendmentRejectionMessageView]
    ...
    "onSubmit" - {
      ...
      "must redirect to declaration summary page on success of handleErrors" in {
        forAll(arbitrary[CC022CType], nonEmptyString) {
          (message, nextPage) =>
            when(mockDepartureP5MessageService.getMessage[CC022CType](any(), any())(any(), any(), any()))
              .thenReturn(Future.successful(message))
            when(mockDepartureP5MessageService.getDepartureReferenceNumbers(any())(any(), any()))
            ...
            when(mockConfig.departureFrontendTaskListUrl(any()))
              .thenReturn(nextPage)

            val request = FakeRequest(POST, declarationAmendmentRejectionMessageOnSubmitRoute)
