...
class MongoDraftDepartureServiceSpec extends SpecBase {

  val mockConnector: DeparturesDraftsP5Connector = mock[DeparturesDraftsP5Connector]
  val service                                    = new MongoDraftDepartureService(mockConnector)
  ...
  "MongoDraftDepartureService" - {
    ...
    "sortDraftDepartures without LRN" - {
      "the connector returns a valid DeparturesSummary" in {
        ...
        whenReady(service.sortDraftDepartures(sortParams, limit, skip)) {
          result =>
            result must be(Some(mockDeparturesSummary))
        }
    ...
    "sortDraftDepartures with LRN" - {
      "the connector returns a valid DeparturesSummary" in {
        ...
        whenReady(service.sortDraftDepartures(sortParams, limit, skip, lrn)) {
          result =>
            result must be(Some(mockDeparturesSummary))
        }
    ...
    "getLRNs with skip and limit" - {
      "the connector returns a valid DeparturesSummary" in {
        ...
        whenReady(service.getLRNs(partialLRN, skip, limit)) {
          result =>
            result must be(Some(mockDeparturesSummary))
        }
    ...
    "getPagedDepartureSummary" - {
      "the connector returns a valid DeparturesSummary" in {
        ...
        whenReady(service.getPagedDepartureSummary(limit, skip)) {
          result =>
            result must be(Some(mockDeparturesSummary))
        }
