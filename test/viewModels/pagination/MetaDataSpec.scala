    ...
    "apply" - {
      "totalNumberOfPages" - {
        ...
        "must return total number of movements divided by number of movements and add 1 if its not whole" in {
          ...
          MetaData(3, 2, 1).totalPages mustEqual 2
          MetaData(5, 2, 1).totalPages mustEqual 3
          MetaData(10, 3, 1).totalPages mustEqual 4
        }

        "must return total number of movements divided by number of movements and not add 1 if its whole" in {

          MetaData(2, 1, 1).totalPages mustEqual 2
          MetaData(6, 2, 1).totalPages mustEqual 3
          MetaData(8, 2, 1).totalPages mustEqual 4
        }
      }
      ...
      "to" - {
        ...
        "must return the number of movements per page if current page is 1" in {
          ...
          MetaData(20, 10, 1).to mustEqual 10
          MetaData(40, 20, 1).to mustEqual 20
          MetaData(100, 50, 1).to mustEqual 50
        }
        ...
        "must return the total number of movements" +
          " when number of movement times by the current page number is greater than the total number of movements" in {
            ...
            MetaData(11, 10, 2).to mustEqual 11
            MetaData(21, 10, 3).to mustEqual 21
            MetaData(101, 50, 3).to mustEqual 101
          }
      }
      ...
      "from" - {
        ...
        "must return 1 when current page is 1" in {
          ...
          MetaData(2, 1, 1).from mustEqual 1
        }

        "must return number of movements per page + 1 when current page is 2" in {

          MetaData(30, 10, 2).from mustEqual 11
          MetaData(5, 2, 2).from mustEqual 3
          MetaData(150, 50, 2).from mustEqual 51
        }

        "must return (number of movements per page times by (current page minus 1)) plus 1 when on any other page" in {

          MetaData(30, 10, 3).from mustEqual 21
          MetaData(10, 2, 4).from mustEqual 7
          MetaData(400, 50, 5).from mustEqual 201
        }
      }
    }
  }
