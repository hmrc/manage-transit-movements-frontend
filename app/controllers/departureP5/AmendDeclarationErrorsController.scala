...
import generated.Generated_CC022CTypeFormat

class AmendDeclarationErrorsController @Inject() (
  override val messagesApi: MessagesApi,
  actions: Actions,
  cc: MessagesControllerComponents,
  messageRetrievalAction: DepartureMessageRetrievalActionProvider,
  viewModelProvider: AmendDeclarationErrorsViewModelProvider,
  view: AmendDeclarationErrorsView
  ...
  def onPageLoad(departureId: String, messageId: String): Action[AnyContent] =
    (Action andThen actions.identify() andThen messageRetrievalAction[CC022CType](departureId, messageId)) {
      implicit request =>
