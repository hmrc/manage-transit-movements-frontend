  ...
  def isDeclarationCancelled(departureId: String, localReferenceNumber: LocalReferenceNumber): Action[AnyContent] =
    (Action andThen actions.checkP5Switch() andThen departureCancelledActionProvider(departureId)) {
      implicit request =>
        ...
        if (isCancelled == "0") {
          Redirect(controllers.departureP5.routes.DepartureCancelledP5Controller.declarationNotCancelled(departureId, localReferenceNumber))
        } else {
          Redirect(controllers.departureP5.routes.DepartureCancelledP5Controller.declarationCancelled(departureId, localReferenceNumber))
        }
    }
}
