...
class ReviewDepartureErrorsP5Controller @Inject() (
  ...
  def onPageLoad(page: Option[Int], departureId: String, messageId: String, isAmendmentJourney: Boolean): Action[AnyContent] =
    (Action andThen actions.checkP5Switch() andThen messageRetrievalAction[IE056Data](departureId, messageId)).async {
      implicit request =>
        ...
        val paginationViewModel = ListPaginationViewModel(
          ...
          href = controllers.departureP5.routes.ReviewDepartureErrorsP5Controller.onPageLoad(None, departureId, messageId, isAmendmentJourney).url
        )
